...
public class LoginController implements ServletContextAware {
	...
	public String RegisterPost(@RequestParam String name, @RequestParam String userName, @RequestParam String password,
			@RequestParam String email, @RequestParam String phone, @RequestParam boolean enabled,
			@RequestParam PRIVILAGE privilage, @RequestParam String streetName, @RequestParam String apptNo,
			@RequestParam String city, @RequestParam String state) {
		Service.addNewUser(name, userName, password, email, phone, enabled, privilage, streetName, apptNo, city, state,
				path);
		return "register";
	}
	...
	public String AddProductPage(Model mo) {
		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
	...
	public String AddProductPost(@RequestParam String name, @RequestParam String code, @RequestParam String category,
			@RequestParam CONDITION condition, @RequestParam long price, @RequestParam long unitsInStock,
			@RequestParam String description, @RequestParam String manufacturer, Model mo) {
		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
	...
	public String ProductDetailPost(Model model, HttpServletRequest request, HttpServletResponse response,
	...
	/*
	 * @RequestMapping(value = "/admin/customer-management", method =
	 * RequestMethod.GET) public String getCustomerList(Model
	 * mo, @RequestParam(defaultValue = "1") int pageNum,
	 * 
	 * @RequestParam(defaultValue = "5") int pageSize) { User u = new User(-1,
	 * "Jon Smith", "JonJonJon", "12345", "jon@jon.jon", "5550142", true,
	 * User.PRIVILAGE.ADMIN, new Address("", "", "", ""));
	 * mo.addAttribute("customers", new User[] { new User(-1, "Jon Smith",
	 * "JonJonJon", "12345", "jon@jon.jon", "5550142", true, User.PRIVILAGE.ADMIN,
	 * new Address("", "", "", "")), new User(-2, "Betty White", "Bdubs", "hello",
	 * "b@w.jon", "5550143", true, User.PRIVILAGE.STANDARD, new Address("", "", "",
	 * "")), new User(-3, "Richard Feynmann", "rfeyn", "r@f.m", "physics_roxx",
	 * "5550144", false, User.PRIVILAGE.STANDARD, new Address("", "", "", "")) });
	 * mo.addAttribute("user", u); mo.addAttribute("pageSize", pageSize);
	 * mo.addAttribute("pageNum", pageNum); return "customer-list"; }
	 * 
	 * @RequestMapping(value = "/admin/customer-management", method =
	 * RequestMethod.POST) public String postCustomerList(Model m) {
	 * m.addAttribute("result", true); return getCustomerList(m, 1, 5); }
	 */
